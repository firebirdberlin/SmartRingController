...
public class TTSService extends Service {
	  ...
	  if (intent.hasExtra(BLUETOOTH_TIMEOUT_EXTRA)) {
		...
		if (tts == null) {
		  if (! preferSco &&
			...
			Logger.i(LOG_TAG, "Starting SCO, will wait until it is connected. sco on: " +
				...
				public void onUtteranceCompleted(String utteranceId) { // ready reading
				  ...
				  synchronized (messageQueue) {
					...
					if (messageQueue.isEmpty()) { //another message to speak ?
					  ...
					  tts = null;
					  TTSService.this.stopSelf();
					  Logger.i(LOG_TAG, "Nothing else to speak. Shutting down TTS, stopping service.");
					} else {
					  Logger.i(LOG_TAG, "Speaking next message.");
  ...
  private void speak(final String text) {
	...
	Logger.i(LOG_TAG, "speaking \"" + text + "\"");
  ...
  private void prepareAudio() {
	...
	if (desiredVolume != -1) {
	  ...
	  Logger.i(LOG_TAG, "Temporarily setting volume to " + boudedDesiredVolume);
